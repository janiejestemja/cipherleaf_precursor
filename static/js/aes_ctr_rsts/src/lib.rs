...
impl AesCtrSecret {
    ...
    pub fn new(key: &[u8], nonce: &[u8]) -> Result<AesCtrSecret, JsValue> {
        if key.len() != 32 {
            return Err(JsValue::from_str("Key must be 32 bytes (256 bits)"));
        }
        if nonce.len() != 8 {
            return Err(JsValue::from_str("Nonce must be 8 bytes (64 bits)"));
        }
        ...
        Ok(AesCtrSecret { key: key_array, nonce: nonce_array, })
    }

    pub fn encrypt(&self, data: &[u8]) -> Vec<u8> {
        aes_ctr(data, &self.key, &self.nonce)
    }
}
